  ...
  try {
    ...
    const pages = await db.query.contentPages.findMany({
      ...
      with: {
        blocks: {
          orderBy: (
            blocks: { sortOrder: unknown },
            { asc }: { asc: (field: unknown) => unknown }
          ) => [asc(blocks.sortOrder)],
        },
      },
    });
...
export async function POST(request: NextRequest) {
  try {
    ...
    const existing = await db.query.contentPages.findFirst({
      where: and(eq(contentPages.slug, slug), eq(contentPages.lang, lang)),
    });

    if (existing) {
      return NextResponse.json({ error: 'Página já existe para este idioma' }, { status: 400 });
    }
