...
export async function GET(req: NextRequest) {
  ...
  let orderRes: unknown[] = []
    if (orderId) {
      ...
      orderRes = await db.select().from(orders).where(eq(orders.stripePaymentIntentId, String(paymentIntent))).limit(1)
    }
  ...
  type OrderRow = { id: string; email: string; paymentStatus?: string | null; status?: string | null; total: string; createdAt: string }
  const order = orderRes[0] as OrderRow
    ...
    const products = await Promise.all(
      items.map(async item => {
        ...
        let file = null
        if (item.variationId) {
          const f = await db.select({ path: files.path }).from(files).where(eq(files.variationId, item.variationId)).limit(1)
          file = f[0]
        }
        if (!file) {
          const f = await db.select({ path: files.path }).from(files).where(eq(files.productId, item.productId)).limit(1)
          file = f[0]
        }
    ...
    try {
      ...
      console.log('📧 Reenvio de confirmação enviado para', order.email, 'resendResult=', resendResult)
